...
class _TripManagementScreenState extends State<TripManagementScreen> {
  final List<TripRequest> _activeTrips = [];
  ...
  void _loadTrips() {
    _activeTrips.add(TripRequest(
      id: 'TR001', route: 'Chennai → Bangalore', passengerCount: 24,
      pickup: 'CMBT Bus Station', dropoff: 'Mysore Road Bus Stand', distance: 350, fare: 1200.0,
      time: '06:00', status: 'Boarding', busNumber: 'TN-01 XY 1234', driverName: 'Suresh Kumar',
      departureTime: DateTime.now().add(const Duration(hours: 1)), arrivalTime: DateTime.now().add(const Duration(hours: 7)), passengers: [],
    ));
    _activeTrips.add(TripRequest(
      id: 'TR002', route: 'Delhi → Agra', passengerCount: 28,
      pickup: 'Kashmiri Gate', dropoff: 'Idgah Bus Stand', distance: 230, fare: 800.0,
      time: '08:30', status: 'Boarding', busNumber: 'DL-02 AB 5678', driverName: 'Amit Sharma',
      departureTime: DateTime.now().add(const Duration(hours: 2)), arrivalTime: DateTime.now().add(const Duration(hours: 5)), passengers: [],
    ));
    _activeTrips.add(TripRequest(
      id: 'TR003', route: 'Mumbai → Pune', passengerCount: 26,
      pickup: 'Dadar Bus Depot', dropoff: 'Swargate', distance: 150, fare: 500.0,
      time: '09:45', status: 'Boarding', busNumber: 'MH-12 CD 9012', driverName: 'Vijay Patil',
      departureTime: DateTime.now().add(const Duration(hours: 3)), arrivalTime: DateTime.now().add(const Duration(hours: 6)), passengers: [],
    ));
    _activeTrips.add(TripRequest(
      id: 'TR004', route: 'Kolkata → Durgapur', passengerCount: 29,
      pickup: 'Esplanade', dropoff: 'Durgapur Bus Terminal', distance: 180, fare: 450.0,
      time: '10:15', status: 'Boarding', busNumber: 'WB-04 EF 3456', driverName: 'Rajan Chatterjee',
      departureTime: DateTime.now().add(const Duration(hours: 4)), arrivalTime: DateTime.now().add(const Duration(hours: 7)), passengers: [],
    ));
    _activeTrips.add(TripRequest(
      id: 'TR005', route: 'Bengaluru → Mumbai', passengerCount: 32,
      pickup: 'Majestic Bus Station', dropoff: 'Dadar Bus Depot', distance: 980, fare: 1500.0,
      time: '22:00', status: 'Boarding', busNumber: 'KA-01 AB 1234', driverName: 'Rajesh Kumar',
      departureTime: DateTime.now().add(const Duration(hours: 2)), arrivalTime: DateTime.now().add(const Duration(hours: 14)), passengers: [],
    ));
  }

  int _getOnboardedCountForTrip(String tripId) {
    final service = RealTimeOnboardingService();
    return service.getOnboardedPassengers(tripId).length;
  }
    ...
    return Scaffold(
      ...
      body: ListView.builder(
        ...
        itemBuilder: (context, index) {
          ...
          final onboardedCount = _getOnboardedCountForTrip(trip.id);
            ...
            child: Container(
              ...
              child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Row(
                  ...
                  children: [
                    Text(trip.route, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
                    if (onboardedCount > 0)
                      Container(
                                                padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 5),
                        decoration: BoxDecoration(
                ...
                const SizedBox(height: 8), 
                Text('Trip #${trip.id}', style: TextStyle(fontSize: 12, color: Colors.grey[600])),
                const SizedBox(height: 16), 
                Row(children: [
                  Expanded(child: Row(children: [
                    Icon(Icons.bus_alert, size: 16, color: Colors.grey[600]), 
                    const SizedBox(width: 8), 
                    Text(trip.busNumber)
                  ])),
                  Expanded(child: Row(children: [
                ...
                Row(children: [
                  Expanded(child: Row(children: [
                    ...
                    const SizedBox(width: 8), 
                    Text('${trip.passengerCount}/35 pre-filled (includes Daily Pass)')
                  ])),
                  Expanded(child: Row(children: [
                ...
                const SizedBox(height: 16),
                //===========================================
                // NAVIGATION TO SCREEN 1: PassengerOnboardingScreen
                //===========================================
                SizedBox(
                  width: double.infinity,
                  child: ElevatedButton.icon(
                    onPressed: () {
                      Navigator.push(
                        ...
                        MaterialPageRoute(builder: (context) => PassengerOnboardingScreen(
                          currentTrip: trip, 
                          currentPassengers: onboardedCount, 
                          maxCapacity: 35,
                        )),
                      ).then((value) {
                        if (value != null) {
                          setState(() {}); // Refresh to show updated count
                          ScaffoldMessenger.of(context).showSnackBar(
                            SnackBar(content: Text('$value passengers onboarded'))
                          );
                        }
                      });
                    },
                    icon: const Icon(Icons.person_add), 
                    label: Text(onboardedCount > 0 ? 'Continue Onboarding' : 'Start Onboarding'),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: onboardedCount > 0 ? Colors.blue[700] : Colors.green[700], 
                      foregroundColor: Colors.white,
...
//===========================================
// USER TYPE SELECTION - MAIN ENTRY POINT
//===========================================
        ...
        child: SafeArea(
          child: Column(children: [
            ...
            Expanded(flex: 2, child: Container(
              ...
              child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [
                const Text('Continue as', style: TextStyle(fontSize: 16, color: Colors.grey)),
                const SizedBox(height: 24),
                //===========================================
                // NAVIGATION TO DRIVER VIEW → TRIP MANAGEMENT → SCREEN 1
                //===========================================
                SizedBox(
                  width: double.infinity, 
                  child: ElevatedButton.icon(
                    ...
                    label: const Text('Driver / Operator', style: TextStyle(fontSize: 16)),
                    style: ElevatedButton.styleFrom(
                      backgroundColor: Colors.blue[800], 
                      foregroundColor: Colors.white, 
                      padding: const EdgeInsets.symmetric(vertical: 16), 
                ...
                SizedBox(
                  ...
                  child: ElevatedButton.icon(
                    onPressed: () {
                      ScaffoldMessenger.of(context).showSnackBar(
                        const SnackBar(content: Text('Passenger view - Select seat and board'))
                      );
                    },
                    icon: const Icon(Icons.person), 
                    label: const Text('Passenger', style: TextStyle(fontSize: 16)),
                    style: ElevatedButton.styleFrom(
